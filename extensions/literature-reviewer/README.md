...
- Main extension: at most one active reviewer; four total dispatches per extension process lifetime.
- Each reviewer: at most two active children, four total child dispatches per process lifetime.
- Thus a single active review tree contains at most three review processes, excluding the coordinating main Pi process; at most five review processes are launched across that tree's lifetime.
...
- Caps are local to an extension process, not a cross-session global quota. Reload creates a new budget. Independent Pi sessions have separate budgets.
